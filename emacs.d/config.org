...
#+BEGIN_SRC emacs-lisp
  (defun worklog-list-files ()
    ...
    (with-current-buffer (get-buffer-create "*Journal Entries*")
      (let ((file-list (mapcar (lambda (s) (substring s (length "/Users/emieri/Dropbox/Worklog/")))
                               (directory-files-recursively "~/Dropbox/Worklog" ".*\.org$"))))
        (org-mode)
        (erase-buffer)
        (insert (string-join file-list "\n")))))
